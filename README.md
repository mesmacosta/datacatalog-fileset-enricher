...
This python package, uses the GCS ````list_buckets```` and ````list_blobs```` APIs to extract the metadata that matches the file pattern and generate file statistics, so their billing policies will apply.
